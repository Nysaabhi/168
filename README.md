...
processFlightBookingQuery(query) {
  ...
  // Enhanced stock patterns to catch more variations of stock-related queries
  const stockPatterns = [
    // Basic stock price queries
    /(?:check|view|what is|show|get|tell me)?\s*(?:the\s*)?([\w\s-]+?)\s*(?:stock|share)?\s*(?:price|rate|value|market value)?$/i,
    
    // "Price of X stock/share" format
    /(?:price|rate|value)\s+(?:of|for)\s+(?:the\s*)?([\w\s-]+?)\s*(?:stock|share)?$/i,
    
    // "X stock/share price" format
    /([\w\s-]+?)\s+(?:stock|share)\s*(?:price|rate|value)?$/i,
    
    // Direct stock symbol queries
  ...
  for (const pattern of stockPatterns) {
    ...
    if (match) {
      // Extract the stock name/symbol and clean it up
      let stock = match[1].trim();
      
      // Remove common words that might appear in the query
      stock = stock.replace(/\b(the|current|latest|live|real-time|realtime|time|now)\b/g, '').trim();
      
      // If the stock name is empty after cleaning, skip this match
      if (!stock) continue;
      
      // Convert to uppercase for consistency
      const stockSymbol = this.normalizeStockSymbol(stock);
      
      // Create the response
      const url = `https://upstox.com/stocks/${encodeURIComponent(stockSymbol)}`;
      return this.createStockResponse(stockSymbol, url);
    }
  }
...
// Helper function to normalize stock symbols
normalizeStockSymbol(stock) {
  // Remove any special characters except '&' and '-'
  const cleaned = stock.replace(/[^a-zA-Z0-9&-\s]/g, '')
                      .replace(/\s+/g, ' ')
                      .trim()
                      .toUpperCase();
  ...
  // Remove spaces and check if we have a mapping
  const noSpaces = cleaned.replace(/\s/g, '');
  return symbolMap[noSpaces] || cleaned;
}
...
createFlightBookingResponse(from, to, departureDate, returnDate, passengers, travelClass, url) {
  ...
  return `
    <div class="stock-query-response">
      <p>📈 Stock Information:</p>
      <ul>
        <li>Symbol: ${stock}</li>
      </ul>
      <button class="click-button" data-url="${url}">
        <i class="fas fa-chart-line"></i> Check Stock Price
      </button>
    </div>
  `;
}
...
.stock-query-response button:hover {
  background: linear-gradient(135deg, #1d4ed8, #1e40af);
  transform: translateY(-1px);
}
      `;
